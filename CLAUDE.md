...
`dist/stage.sh` is the single package-content implementation used by both `PKGBUILD.package()` and the dpkg/plain paths. GUI packages therefore stage the same five identities, icons, portal files, and shared assets. Non-Arch paths also build and package `sketerm-webengine` when `CEF_INCLUDE` and `CEF_LIB` identify a system CEF header/runtime pair (defaults `/usr/include/cef` and `/usr/lib/cef`); without one, the browser identity still ships but the installer warns explicitly that the helper is omitted. On dpkg hosts, `--deps` installs GUI development packages before the eligibility probe for GUI/auto mode. Without `--deps`, `--gui-only` fails on missing/old dependencies and auto mode degrades to mux-only with a warning.
