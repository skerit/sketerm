...
**Wayland app forwarding (proto v2).** Byte channels (`chan_open/data/close` wire frames) multiplex generic streams over the mux connection — any transport, so forwarded apps roam with UDP. Daemon: each session's command is wrapped in `waypipe server` (when waypipe exists on the host; `SKETERM_MUX_NO_WAYLAND=1` disables), giving the shell a per-session `$WAYLAND_DISPLAY`; app connections land on a per-session hub socket and become channels toward the latest attached proto>=2 client. GUI: `src/wlbridge.zig` runs ONE `waypipe client` against the local compositor; `terminal.zig` pumps channels into it. BOTH waypipe ends need `--no-gpu` when their host lacks a Vulkan ICD (waypipe 0.11 aborts otherwise) — checked at spawn on each side. `sketerm app <host> <cmd>` (src/remoteapp.zig) is the standalone non-mux path over plain SSH.
