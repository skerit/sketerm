...
**Root assignment is enforced, not trusted.** `dist/test-test-roots.sh` runs as a dependency of `test`, `test-core` and `test-web`: it finds every `src/**.zig` carrying a `test` block and fails the build unless the right roots import it — core modules by BOTH `tests.zig` and `tests_core.zig`, `ui/`/`render/` and the listed GUI-dependency outliers by `tests.zig` only, and CEF-typed code by `src/webengine.zig` (the `zig build test-web` root, which needs a CEF install) and by neither other root. Duplicate imports fail too. So a test block in a module nobody imports is a build error rather than a test that silently never runs, and a GTK-free exception under `ui/` is one edit to that script's classification lists.
...
`build.zig` defines a bool `build_options.glib`. GUI targets get `true`; `sketerm-mux` and `smoke-mux` get `false` plus the lean `configureCoreDeps` dependency set (libc only). It gates GLib-only code inside modules BOTH dependency sets compile; today that is `ui/panel/assets.zig`'s GObject refcounting. `src/pty.zig` is GLib-free (only daemons own PTYs): the per-Pty write queue (`WRITE_QUEUE_CAP`, 1 MiB) is drained by `flushQueue` when the daemon's poll loop sees POLLOUT on the session entry (`Daemon.tick` -> `flushSessionInput`). `writeAll` never blocks or spins; the only refusal is the cap, reported through `WriteResult.first_drop` / `Flush.drained` so the daemon logs it (a spin-then-drop fallback silently truncated pastes into a slow child once). Anything imported by the mux side (`src/mux/*`, parser, grid) must stay free of GTK/GLib references.
...
**Helpers worth knowing.** `cast.userData(T, user)` in `src/util/cast.zig` collapses `@ptrCast(@alignCast(user.?))` for GTK callbacks. `style.colorToVec` / `style.colorToRGBA` in `src/render/style.zig` are the shared color-resolution path used by both `cell_pass` and `grid_pass` — don't copy that logic into a third place. The same file also owns ALL SGR line-decoration geometry — rects (`decoThin`/`decoStrip`/`decoDoubleLines`) and the undercurl wave (`CURLY_*`/`curlyStripHeight`) — plus `DECO_GLSL`, the one GLSL mirror generated from those very constants and prepended to BOTH shader pairs (`sk_decoStrip`, `sk_curlyCoverage`). CellPass places its strip in-shader while GridPass emits quads on the CPU, so a hand-written rect or a second wave formula in either pass is a visible seam on rows that switch passes; `smoke-cell`'s deco + curly stages assert per-scanline and per-column parity. `src/ui/webframe.zig` is the same deal for browser frame presentation (refcounted `Map`, `buildBgraTexture`, `decodeInlineRect`, `cefButton`/`modsOf`/`modsFromState`), shared by `webface.zig` and `webaction.zig`. `input.matchWithDefaults` is the one binding lookup (pane table or defaults, lowered-then-raw keyval) used by every face that dispatches global actions — `viewer.zig` included. `src/ui/debounce.zig` is the one debounced-write state machine: its `teardown()` FLUSHES a pending write instead of dropping it, which is why the tree sidebar and the file browser both keep a width changed in the last 400ms before close. `src/util/pathz.zig` owns path helpers (`makeParentDirs`). Each of these exists because the logic HAD been written twice.
...
- **`src/mux/CLAUDE.md`** — the session daemon, wire protocol, transports,
  ...
  - The GUI holds no `Pty` at all (a `Terminal` is only a daemon client).
    ...
    without it a reaped pty would `waitpid(-1)` and reap another session's child.
...
- An `Event`'s OSC/APC/DCS body is a heap slice the event owns; whoever applies or sends it frees it with `Event.deinit`. There is no parse arena.
...
- **Do not combine 1 and 2 — that combination is worse than either alone.** A `GDestroyNotify` on a signal connection runs when the CLOSURE is destroyed, and an explicit disconnect destroys the closure: the disconnect *is* the free. Any code that expected the data to survive the disconnect then has a use-after-free. (Qdata-at-finalize plus a disconnect is safely combinable, because there the free is tied to the object's finalize, not to the closure — `panelwin.zig` relies on that ordering.) The rule forbids *expecting the data to outlive the disconnect*, not the disconnect itself: deliberately disconnecting IN ORDER to free is correct and is how `tabsidebar.zig`'s `pruneTitleWatches` drops a per-page title watch — the `GDestroyNotify` runs, unregisters the context and releases the sidebar ref. Write it that way only where the notify is the single owner and the caller treats the context as gone immediately after. Where one context is user-data for several connections, exactly one owner: `panel/view.zig`'s `Compare` is owned by its qdata notify and *borrowed*, notify-free, by its four gesture controllers.
...
- GTK signal contexts allocated on the heap (`*Ctx` types) **must** carry their own `allocator: std.mem.Allocator` and pass a matching destroy callback as `g_signal_connect_data`'s 5th arg. When the context owns nothing but itself that callback is `cast.destroyCtx(T)` (`src/util/cast.zig`) — do not hand-write another "resolve, then `allocator.destroy`" fn; every `add*Row` builder in `prefs.zig` and `menu.zig`'s `ActionSlot`/`HoverCtx` connections use it. Write a custom `freeXxxCtx` only when the context also owns slices or references that must be released first (`menu.zig`'s `freeClickCtx`, `browser/locbar.zig`'s `PathCtx.free`). Skip the destroy-notify only when the user-data is the dialog's main `Ctx` (managed elsewhere) or is a non-heap pointer.
...
- **A harness that starts a daemon out of process arms `lifetime.arm()` first** (`src/util/lifetime.zig`): a pipe whose read end is published as `$SKETERM_MUX_LIFETIME_FD` and inherited by every descendant, while only the harness holds the write end. `sketerm-mux` polls it and shuts down on EOF, so every broker below the harness (the one it spawned, the one `sketerm mcp` or a GUI autostarted, their workers) retires when the harness dies by ANY path, SIGKILL and panics included. Before this, 53 brokers from one day's smoke runs and a dozen from a consumer's IT suite were alive at night. `platform.dieWithParent` on exec'd children stays as the SIGKILL backstop for a wedged daemon; a forked child that does not exec must `dropWriteEnd()`.
...
- **macOS isolation needs an explicit socket, not just XDG vars.** On Darwin the runtime dir derives from `TMPDIR`, so isolating `XDG_CONFIG_HOME`/`XDG_STATE_HOME`/`SKETERM_APP_ID` per the recipe above is NOT enough: an "isolated" GUI still autostart-connects to the user's REAL daemon and its durable sessions (observed 2026-08-20 — an agent's test session appeared in the user's live mux.log). Always pass an explicit short `--socket` path (and point the GUI at it) on macOS.
- A `zig build test` run can fail spuriously in the OCR (tesseract) tests with leak warnings; rerun once before diagnosing.
- **`sketerm doctor` lists every sketerm process of the current user** (`src/procinv.zig`): daemons with the socket they should serve, session workers with their session name, file jobs, keepers, GUI subcommands, browser helpers. It identifies by executable, so it also finds daemons started before 0.1.5, which re-exec'd via `/proc/self/exe` with argv[0] `/proc/self/exe` and comm **"exe"** — invisible to `ps ax | grep sketerm` and `pgrep -x sketerm-mux` (a stale test daemon once silently kept serving an old binary for an hour this way). Every self-exec now passes argv[0] `sketerm-mux` and `mux_main` renames the process (`src/mux/selfexec.zig`). A daemon whose socket file is gone or answered by another daemon is a doctor WARNING: its sessions are alive and unreachable. When hunting rig leftovers, start with doctor under the isolated env, then match `/proc/<pid>/environ` against the isolated dir.
...
`dist/stage.sh` is the single implementation of version derivation (`sketerm_pkgver`), the build recipe (`sketerm_build`) and the package content (`sketerm_stage`), used by both `PKGBUILD` and the dpkg/plain paths — each caller is one line, and `dist/test-install.sh` asserts the two backends still agree. Only the CEF paths differ: `install.sh` passes the probed `$CEF_INCLUDE`/`$CEF_LIB`, while `PKGBUILD` passes the distro locations as literals so the Arch package cannot be redirected by the caller's environment. GUI packages therefore stage the same five identities, icons, portal files, and shared assets. Non-Arch paths also build and package `sketerm-webengine` when `CEF_INCLUDE` and `CEF_LIB` identify a system CEF header/runtime pair (defaults `/usr/include/cef` and `/usr/lib/cef`); without one, the browser identity still ships but the installer warns explicitly that the helper is omitted. On dpkg hosts, `--deps` installs GUI development packages before the eligibility probe for GUI/auto mode. Without `--deps`, `--gui-only` fails on missing/old dependencies and auto mode degrades to mux-only with a warning.
...
Five desktop entries and five app icons ship for the terminal, file manager, viewer, editor, and browser because **each mode is its own application identity**. `sketerm files` registers the GApplication id `dev.sker.sketerm.files` and sets the matching prgname, so on Wayland the toplevel app_id and on X11 the WM_CLASS are that string and KDE gives the file manager its own taskbar entry and icon (a per-window `gtk_window_set_icon_name` cannot do this). `data/dev.sker.sketerm.desktop` + `data/icons/hicolor/scalable/apps/dev.sker.sketerm.svg` are the terminal; `data/dev.sker.sketerm.files.desktop` + `data/icons/hicolor/scalable/apps/dev.sker.sketerm.files.svg` are the file manager, and the latter declares `MimeType=inode/directory;x-scheme-handler/file;` so it can be set as the default file manager. `data/dev.sker.sketerm.web.desktop` + `data/icons/hicolor/scalable/apps/dev.sker.sketerm.web.svg` are the browser (`sketerm web [url]`, or the `sketerm-web` hardlink), declaring `x-scheme-handler/http;x-scheme-handler/https;text/html;` so it can be set as the default browser; its `Exec=sketerm-web %U` runs the argv0 identity hardlink `dist/stage.sh` creates beside the files/viewer/editor ones, which is free to carry that name because the CEF helper is `sketerm-webengine` and only the exact basename is matched. `StartupWMClass` in each entry MUST stay equal to its app id. **Every SVG under `data/icons/hicolor/scalable/{apps,actions}` is staged by glob, never by a hand-written list, and `dist/test-install.sh` drift-tests both directions** — every `sketerm-*-symbolic` name `src/` asks for has a file, and every bundled file reaches both backends' staged trees (a list is how the bookmark button shipped as the text "Bookmark"). This is additive: a browser face on a pane inside a terminal window (palette `new_browser_tab`/`new_browser_split`, `cli new-browser-tab`, `sketerm files --here|--tab`) is unrelated to the files identity and must keep working. Terminfo source at `terminfo/sketerm-256color.src` — `tic`-compiled into `/usr/share/terminfo` by the PKGBUILD.
