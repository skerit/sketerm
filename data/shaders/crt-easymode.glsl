/*
    ...
    (libretro/glsl-shaders crt/shaders/crt-easymode.glsl).
    Port changes: RetroArch's Texture/TextureSize/InputSize/
    OutputSize plumbing mapped onto sketerm's iChannel0/iResolution;
    the vertex stage (a plain passthrough) folded away; a `v_scale`
    parameter added — these shaders are written for low-res emulator
...
//@name CRT EasyMode
//@desc EasyMode's flat CRT (RetroArch classic, GPL): lanczos-sharpened pixels, beam-width scanlines, RGB mask. v_scale sets the virtual source resolution.
...
#pragma parameter SCANLINE_CUTOFF "Scanline Cutoff" 1000.0 1.0 1000.0 1.0
...
#define TEX2D(c) dilate(texture(iChannel0, c))
...
{
    // Port plumbing: the "source" is the terminal frame at
    // iResolution / v_scale virtual pixels.
    vec2 SourceSizeXY = iResolution.xy / v_scale;
    vec4 SourceSize = vec4(SourceSizeXY, 1.0 / SourceSizeXY);
    ...
    vec2 mod_fac = floor(fragCoord / vec2(MASK_SIZE, MASK_DOT_HEIGHT * MASK_SIZE));
