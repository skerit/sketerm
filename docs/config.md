...
| `web_route` | string | `direct` | Default network route for new browser tabs whose container has no route of its own: `direct`, `tor`, `via:<host>` (egress: traffic leaves from that mux/SSH host, DNS resolves there), or `on:<host>` (the browser process runs on that host). `tor` dials `mux_tor_socks_endpoint`. A route is realized as a separate browser process, so identity is re-shared by cookie sync between the routes whose browser runs on this machine (`direct`, `tor`, `via:`); an `on:<host>` browser keeps its own cookie jar on that host, and cookie values never cross to it. A route whose proxy the browser engine refuses serves nothing at all (the tab says why) rather than browse outside it, and `tor`/`via:` also keep WebRTC off non-proxied UDP. Validated at load; a per-tab override is the route button beside the address bar (or the palette's `web_route_*` actions). Anything else is a parse error. |
...
web_site_info web_history web_bookmarks web_extensions web_userscripts
web_filter_lists close_pane toggle_browser_face
new_editor_tab new_editor_split toggle_editor_face toggle_web_face
toggle_panel_face panel_open panel_open_window panel_close mux_detach
paste_clipboard
