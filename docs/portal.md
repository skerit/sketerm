...
- **Wayland parenting needs a compositor with `xdg_foreign`.**
  sketerm's own compositor advertises `zxdg_exporter_v2` /
  ...
  Cross-connection parents are real now: the daemon brain resolves the
  handle and ships the parent's session-wide identity (its app channel
  id + surface id) to the viewer, which looks the window up across
  `AppHost`s and applies `gtk_window_set_transient_for` -- so a dialog
  whose caller is a different process of the same session is stacked
  and moved by the host compositor like any transient child, and is
  modal over it when the client asked (`xdg_wm_dialog_v1`). The X11
  branch is not exercised by any test in this repo: X sessions are
  off-limits for GUI testing here (see CLAUDE.md), and no X11 desktop
  is available to the suite.
