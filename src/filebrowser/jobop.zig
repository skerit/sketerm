...
//!
//! The verb vocabulary is `fsjob.Op`; the switch below is exhaustive
//! over it, so a verb added to the helper does not compile until this
//! module says what the browser shows for it.
...
pub const JobOp = enum {
    ...
    restore,
    delete,
    /// A search, live query or panelize stream: it fills a tab of its
    /// own and never belongs in the transfers strip.
    query,
    other,

    /// Classify a wire verb. `delete_src` is the cross_copy move flag;
    /// the plain `copy` verb never moves (a same-host move is a rename,
    /// not a job). The inline `delete` fs op is not a job verb but is
    /// phrased like one when a row carries it.
    pub fn fromWire(op: []const u8, delete_src: bool) JobOp {
        if (std.mem.eql(u8, op, "delete")) return .delete;
        const verb = std.meta.stringToEnum(fsjob.Op, op) orelse return .other;
        return switch (verb) {
            .copy, .cross_copy => if (delete_src) .move else .copy,
            .trash => .trash,
...
test "wire verbs classify, and only copies count bytes" {
    ...
    try t.expectEqual(JobOp.delete, JobOp.fromWire("delete", false));
    try t.expectEqual(JobOp.query, JobOp.fromWire("find", false));
    try t.expectEqual(JobOp.query, JobOp.fromWire("live_find", false));
    try t.expectEqual(JobOp.query, JobOp.fromWire("panelize", false));
    try t.expectEqual(JobOp.other, JobOp.fromWire("hash", false));
    ...
    try t.expectEqual(JobOp.other, JobOp.fromWire("flat", false));
    try t.expect(JobOp.copy.countsBytes());
    try t.expect(!JobOp.trash.countsBytes());
    try t.expect(!JobOp.query.inStrip());
    try t.expectEqualStrings("Copying", JobOp.copy.activeVerb());
    try t.expectEqualStrings("Moved", JobOp.move.doneVerb());
}
