...
pub const TOOLS = [_]ToolDef{
    ...
    .{
        ...
        ,
        .output_schema =
        \\{"type":"object","properties":{"backend":{"type":"string","enum":["gui","headless"]},"pane":{"type":"integer","description":"View handle in GUI mode (a real pane id)"},"view":{"type":"integer","description":"View handle in headless mode (a helper view id)"},"origin":{"type":"string"},"url":{"type":"string"},"title":{"type":"string"},"loading":{"type":"boolean"},"policy_exhausted":{"type":"boolean","description":"A network-policy budget latched: reads still answer, new traffic is refused"},"policy_exhausted_reason":{"type":"string"},"settled":{"type":"boolean","description":"The requested navigation finished inside the timeout"},"where_ignored":{"type":"boolean","description":"Headless: 'where' has no placement to apply"},"document":{"type":"integer","description":"Per-document counter; 1 means the view has only ever held THIS page"},"revision":{"type":"integer"},"snapshot":{"type":"string","description":"The semantic tree, same text as the content block"},"snapshot_error":{"type":"string"},"profile":{"type":"string"},"profile_kind":{"type":"string","enum":["default","named","ephemeral"]},"context":{"type":"integer","description":"Engine identity-context id; 0 = the shared default jar"},"policy_active":{"type":"boolean"},"policy_source":{"type":"string","enum":["call","profile_default","none"]},"policy_serial":{"type":"integer"},"policy":{"type":"object","description":"Echo of the effective enforced policy"}},"required":["backend","origin","url","title","loading","settled"]}
        ,
    },
    .{
        .name = "web_close",
    ...
    .{
        ...
        ,
        .input_schema =
        \\{"type":"object","properties":{"pane":{"type":"integer","description":"Tighten this live view's policy"},"view":{"type":"integer","description":"Synonym for 'pane'"},"profile":{"type":"string","description":"Register the session default for this profile name instead"},"policy":{"type":"object","description":"Same shape as web_open's policy object. For a live view it is a PATCH: a field you omit stays exactly as it is (omitting allow_schemes does NOT reset to http+https, omitting allow_private_addresses does NOT turn it off); a field you supply must tighten or it is named under ignored. For a profile default, omitted fields take web_open's defaults."}},"required":["policy"]}
        ,
        .output_schema =
    ...
    .{
        ...
        .mutates = true,
        .description =
        \\Navigate a web view: a 'url', or an 'action' (back|forward|reload|stop). Waits (bounded) for the nav state to settle and returns url/title/loading/can_back/can_fwd — NOT a snapshot. Ask for content separately: web_read to read it, web_snapshot to act on it. Omitting 'pane' means the CURRENT view (web_tabs marks it); passing one switches the default to that view.
        ,
        .input_schema =
        ...
        ,
        .output_schema =
        \\{"type":"object","properties":{"backend":{"type":"string","enum":["gui","headless"]},"pane":{"type":"integer"},"view":{"type":"integer"},"origin":{"type":"string"},"url":{"type":"string"},"title":{"type":"string"},"loading":{"type":"boolean"},"policy_exhausted":{"type":"boolean","description":"A network-policy budget latched: reads still answer, new traffic is refused"},"policy_exhausted_reason":{"type":"string"},"can_back":{"type":"boolean"},"can_fwd":{"type":"boolean"},"settled":{"type":"boolean"}},"required":["backend","origin","url","title","loading","can_back","can_fwd","settled"]}
        ,
    },
    .{
        .name = "web_snapshot",
        ...
        ,
        .output_schema =
        \\{"type":"object","properties":{"backend":{"type":"string","enum":["gui","headless"]},"pane":{"type":"integer"},"view":{"type":"integer"},"origin":{"type":"string"},"url":{"type":"string"},"title":{"type":"string"},"loading":{"type":"boolean"},"policy_exhausted":{"type":"boolean","description":"A network-policy budget latched: reads still answer, new traffic is refused"},"policy_exhausted_reason":{"type":"string"},"kind":{"type":"string","description":"full or delta"},"document":{"type":"integer"},"revision":{"type":"integer"},"snapshot":{"type":"string","description":"The semantic tree, same text as the content block"},"detail":{"type":"integer","description":"Only when this call MOVED the session default"},"unchanged":{"type":"boolean","description":"A delta with an empty body: the page did not change"}},"required":["backend","origin","url","title","loading","kind","document","revision","snapshot"]}
        ,
    },
    .{
        .name = "web_act",
        ...
        .mutates = true,
        .description =
        \\Act on a semantic ID from web_snapshot or web_read: click (a REAL pointer event at the element, so the page sees isTrusted), focus, hover, scroll_into_view, or set_value. IDs returned by web_read are revision-guarded and fail when stale instead of resolving against a changed page. set_value types into a text field with real key events, picks the matching option in a native <select> (by option text or value, including one inside an open shadow root), and opens an ARIA/custom dropdown with a trusted click then clicks the matching [role=option]. The reply echoes WHAT was acted on plus the delta that followed, so a mismatch with what you intended is visible. Omitting 'pane' means the CURRENT view (web_tabs marks it); passing one switches the default to that view.
        ,
        .input_schema =
        ...
        ,
        .output_schema =
        \\{"type":"object","properties":{"backend":{"type":"string","enum":["gui","headless"]},"pane":{"type":"integer"},"view":{"type":"integer"},"origin":{"type":"string"},"url":{"type":"string"},"title":{"type":"string"},"loading":{"type":"boolean"},"policy_exhausted":{"type":"boolean","description":"A network-policy budget latched: reads still answer, new traffic is refused"},"policy_exhausted_reason":{"type":"string"},"id":{"type":"integer"},"action":{"type":"string"},"acted":{"type":"boolean","description":"Always true; a refused act is an isError result naming the page's reason"},"detail":{"type":"string"},"delta_kind":{"type":"string"},"delta":{"type":"string","description":"What changed after the act"},"delta_error":{"type":"string"},"navigated_to":{"type":"string","description":"Only when the act moved the view to another url"},"loading_after":{"type":"boolean"}},"required":["backend","origin","url","title","loading","id","action","acted","detail"]}
        ,
    },
    .{
        .name = "web_expand",
