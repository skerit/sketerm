...
const crosscopy = @import("crosscopy.zig");
...
/// Every verb the `--job` helper runs. The wire verb IS the tag name
/// (`daemon_fsjobs.jobOpFor` is a `stringToEnum`), `serve` dispatches
/// on it and `filebrowser/jobop.zig` classifies it -- one enum, so a
/// verb added here fails to compile until every consumer says what to
/// do with it. Append-only: the names are on the wire.
pub const Op = enum {
    copy,
    delete_tree,
    hash,
...
/// scoped temp name a symlink install goes through (a `-<job>` or
/// `-<pid>` scope is appended; the unscoped form is the legacy name
/// older helpers left behind and current ones still clean up).
pub const PART_SUFFIX = ".skpart";
pub const LINK_PART_SUFFIX = ".skpart-link";
...
pub threadlocal var durable_state: DurableState = .{};
...
pub fn emitError(msg: []const u8) u8 {
...
pub fn emitErrorKind(kind: []const u8, msg: []const u8) u8 {
...
pub fn emitCanceled(msg: []const u8) u8 {
...
pub const Progress = struct {
    ...
    pub fn setFile(self: *Progress, path: []const u8) void {
    ...
    pub fn emitNow(self: *Progress) void {
        self.since_emit = 0;
...
/// The one cancellation probe of a durable job: sticky once seen and
/// never true past the deletion boundary (crosscopy owns the rule).
pub fn cancelRequested(spec: Spec) bool {
    return crosscopy.durableCancelRequested(spec.journal_dir, spec.job_id);
}
...
pub fn serve(allocator: std.mem.Allocator) u8 {
    ...
    const rc: u8 = if (std.meta.stringToEnum(Op, spec.op)) |op| switch (op) {
        .copy => runCopy(allocator, spec),
        .delete_tree => runDeleteTree(spec),
        .hash => runHash(spec),
        ...
        .archive_extract => runArchiveExtractMember(spec),
        .trash => runTrash(spec),
        .trash_restore => runTrashRestore(spec),
        .cross_copy => crosscopy.run(allocator, spec),
        .panelize => runPanelize(spec),
        .live_find => runLiveFind(allocator, spec),
        .dir_size => runDirSize(spec),
        .disk_usage => runDiskUsage(allocator, spec),
        .perm_tree => runPermTree(spec),
        .thumbnail => runPreview(allocator, spec, true),
        .preview => runPreview(allocator, spec, false),
        .preview_transport => runPreviewTransport(allocator, spec),
        .preview_stream => runPreviewStream(spec),
        .media_meta => runMediaMeta(allocator, spec),
        .git_status => runGitStatus(allocator, spec),
        .git_diff => runGitDiff(allocator, spec),
        .diff => runDiff(allocator, spec),
        .split => runSplit(spec),
        .combine => runCombine(spec),
        .secure_delete => runSecureDelete(spec),
    } else emitError("unknown job op");
...
pub fn saveCrossPhase(spec: Spec, phase: []const u8, progress: *const Progress, durable: CrossDurable) bool {
...
pub fn emitCrossPhase(phase: []const u8, progress: *const Progress) void {
    emit(.{
        .ev = "progress",
        .done = progress.done,
        .total = progress.total,
...
pub fn persistCrossPhase(spec: Spec, phase: []const u8, progress: *const Progress, durable: CrossDurable) bool {
...
fn trashRoot(buf: []u8) ?[]const u8 {
    return fsserve.trashRoot(fsserve.envOpt("XDG_DATA_HOME"), fsserve.envOpt("HOME"), buf);
}
...
fn runCopyRootLink(spec: Spec) u8 {
    ...
    const temp = std.fmt.bufPrint(&temp_buf, "{s}" ++ LINK_PART_SUFFIX ++ "-{d}", .{ spec.dst, c.getpid() }) catch
        return emitError("path too long");
...
pub fn emitCopyDone(progress: *const Progress) void {
    emit(.{
        .ev = "done",
        .done = progress.done,
        .total = progress.total,
...
fn copyOneFile(src: []const u8, dst: []const u8, src_st: c.struct_stat, allow_resume: bool, verify: bool, no_replace: bool, progress: *Progress) CopyResult {
    ...
    wpart.print("{s}" ++ PART_SUFFIX, .{dst}) catch return .{ .err = "path too long" };
...
pub fn stampEntry(hash: *Sha256, path: []const u8, kind: []const u8, size: u64, mode: u32, mtime_ns: i64, ctime_ns: i64, dev: u64, ino: u64, target: []const u8) void {
...
fn copyTreeDir(allocator: std.mem.Allocator, src_dir: []const u8, dst_dir: []const u8, opts: CopyOpts, progress: *Progress, root: bool) CopyResult {
    ...
    for (l.entries) |e| {
        ...
        if (std.mem.eql(u8, e.kind, "dir")) {
            ...
            var temp_buf: [4096]u8 = undefined;
            const temp = std.fmt.bufPrint(&temp_buf, "{s}" ++ LINK_PART_SUFFIX ++ "-{d}", .{ dpath, c.getpid() }) catch return .{ .err = "path too long" };
...
test "copyOneFile resumes only on matching prefix hash" {
    ...
    try pw.print("{s}/dst.bin" ++ PART_SUFFIX, .{dir});
