...
fn nativeBacklogStage(allocator: std.mem.Allocator, sock_path: []const u8, wl_id: u32) void {
    var conn = client_mod.Conn.connect(allocator, sock_path) catch fail("bk connect");
    defer conn.deinit();
    const tv = c.struct_timeval{ .tv_sec = 30, .tv_usec = 0 };
    _ = c.setsockopt(conn.fd, c.SOL_SOCKET, c.SO_RCVTIMEO, &tv, @sizeOf(c.struct_timeval));
    conn.sendJson(.hello, .{ .proto = wire.PROTO_VERSION }) catch fail("bk hello");
    (conn.recvExpect(&.{.welcome}) catch fail("bk welcome")).deinit(allocator);
    conn.sendJson(.spawn, .{
        .name = "bkapp",
    ...
    (conn.recvExpect(&.{.ok}) catch fail("bk spawn ok")).deinit(allocator);
    conn.sendJson(.attach, .{ .name = "bkapp", .kind = "mcp" }) catch fail("bk attach");
    (conn.recvExpect(&.{.snapshot}) catch fail("bk snapshot")).deinit(allocator);
    ...
    const app_fd = @import("util/platform.zig").socketCloexec(c.AF_UNIX, c.SOCK_STREAM, 0);
    if (app_fd < 0) fail("bk app socket");
    var addr: c.struct_sockaddr_un = undefined;
    daemon_mod.fillSockaddrUn(&addr, disp) catch fail("bk sockaddr");
    if (c.connect(app_fd, @ptrCast(&addr), @sizeOf(c.struct_sockaddr_un)) != 0) fail("bk app connect");
    defer _ = c.close(app_fd);
    _ = c.setsockopt(app_fd, c.SOL_SOCKET, c.SO_RCVTIMEO, &tv, @sizeOf(c.struct_timeval));
    ...
    var tmp_buf: [128]u8 = undefined;
    const tmp_path = std.fmt.bufPrintZ(&tmp_buf, "/tmp/sketerm-mux-smoke-{d}-bkpool", .{c.getpid()}) catch unreachable;
    const pool_fd = c.open(tmp_path.ptr, c.O_RDWR | c.O_CREAT | c.O_TRUNC, @as(c.mode_t, 0o600));
    if (pool_fd < 0) fail("bk pool open");
    ...
    if (c.pwrite(pool_fd, pool_random.ptr, POOL_SIZE, 0) != POOL_SIZE) fail("bk pool fill");
    ...
    { // get_registry(2), bind shm(3) + compositor(6)
        ...
        if (c.write(app_fd, stream.items.ptr, stream.items.len) != @as(isize, @intCast(stream.items.len))) fail("bk write binds");
    }
    { // wl_shm.create_pool(4, fd, POOL_SIZE)
        var b = wlwire.Builder.init(&mbuf, 3, 0);
        b.putNewId(4);
        b.putInt(POOL_SIZE);
        sendWithFd(app_fd, b.finish() catch unreachable, pool_fd) catch fail("bk create_pool");
    }
    stream.clearRetainingCapacity();
    { // create_buffer(5, 0, 128x128, stride 512, fmt 1) + surface(7) + attach + commit
        ...
        if (c.write(app_fd, stream.items.ptr, stream.items.len) != @as(isize, @intCast(stream.items.len))) fail("bk write setup");
    }
    ...
    // Streaming sanity: the first commit reaches the mcp client.
    {
        ...
        while (!seen_commit and rounds < 400) : (rounds += 1) {
            const f = conn.recvFrame() catch fail("bk first-commit read");
            defer f.deinit(allocator);
            if (f.ftype == .chan_data and std.mem.indexOf(u8, f.payload[4..], &commitNeedle()) != null)
                seen_commit = true;
        }
        if (!seen_commit) fail("bk: first commit never streamed");
    }

    // FLOOD while the client stalls: 250 x 64KB incompressible full
    // copies ≈ 16MB — far past NATIVE_BACKLOG (8MB).
    {
    ...
    if (c.pwrite(pool_fd, final_pat.ptr, POOL_SIZE, 0) != POOL_SIZE) fail("bk final fill");
    {
        var b = wlwire.Builder.init(&mbuf, 7, 6); // commit
        const m = b.finish() catch unreachable;
        if (c.write(app_fd, m.ptr, m.len) != @as(isize, @intCast(m.len))) fail("bk final commit");
    }
    ...
    while (!saw_sync and rounds < 20_000) : (rounds += 1) {
        const f = conn.recvFrame() catch fail("bk drain read (native_sync never arrived)");
        defer f.deinit(allocator);
        switch (f.ftype) {
            ...
            .chan_open => {
                if (saw_gap) {
                    const open = wire.decodeChanOpen(f.payload) orelse fail("bk reopen decode");
            ...
            .chan_data => {
                ...
                var pos: usize = 0;
                while ((wlpipe.peelUnit(replay_units.items[pos..]) catch fail("bk replay unit")) != null) {
                    const p = (wlpipe.peelUnit(replay_units.items[pos..]) catch unreachable).?;
                    switch (p.unit.tag) {
                        .pool_update_c => {
                            const upd = wlpipe.decodePoolUpdateC(p.unit.payload) orelse fail("bk replay update");
                            if (upd.pool == 4 and upd.offset + upd.body.raw_len <= POOL_SIZE) {
                                const dst = shadow[upd.offset..][0..upd.body.raw_len];
                                wlpixcodec.decodeBody(upd.body, dst) catch fail("bk replay decode");
                            }
                        },
    ...
    if (!saw_gap) fail("bk: native_gap never sent (backlog cap not enforced for mcp client)");
    if (!saw_reopen) fail("bk: replay chan_open missing after drain");
    if (!saw_state_sync) fail("bk: replay state_sync missing");
    if (!saw_sync) fail("bk: native_sync never arrived");
    if (!std.mem.eql(u8, shadow, final_pat)) fail("bk: replay pool bytes are not the LIVE mirror (stale screenshot regression)");
    std.debug.print("smoke-mux: mcp backlog gap + live-mirror resync ok\n", .{});

    // Tear the session down; the attached client gets `gone`.
    conn.sendJson(.kill, .{ .name = "bkapp" }) catch fail("bk kill");
    var kr: usize = 0;
    var got_gone = false;
    while (!got_gone and kr < 400) : (kr += 1) {
        const f = conn.recvFrame() catch fail("bk kill read");
        defer f.deinit(allocator);
        if (f.ftype == .gone) got_gone = true;
    }
    if (!got_gone) fail("bk: gone never arrived after kill");
}
...
pub fn main() u8 {
    ...
    // Backlogged mcp client: gap + live-mirror resync ("bkapp" is the
    // second hub session ever → wl-2).
    nativeBacklogStage(allocator, sock_path, 2);
