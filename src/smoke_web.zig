...
const shared_route_view_a: u32 = 13;
const shared_route_view_b: u32 = 14;
...
const Client = struct {
    ...
    ack_contexts_shared_storage: bool = false,
    context_shared_seq: u32 = 0,
    context_shared_req: u32 = 0,
    context_shared_ok: u8 = 0,
    context_shared_reason: [128]u8 = @splat(0),
    context_shared_reason_len: usize = 0,
        ...
        switch (frame.tag) {
            .hello_ack => {
                ...
                for (ack.caps) |cap| {
                    ...
                    if (std.mem.eql(u8, cap, proto.CAP_CONTEXTS_SHARED_STORAGE)) self.ack_contexts_shared_storage = true;
            ...
            .ev_context_create_shared => {
                const e = proto.decode(proto.EvContextCreateShared, frame.payload) catch fail("ev_context_create_shared decode");
                self.context_shared_req = e.req;
                self.context_shared_ok = e.ok;
                self.context_shared_reason_len = @min(e.reason.len, self.context_shared_reason.len);
                @memcpy(self.context_shared_reason[0..self.context_shared_reason_len], e.reason[0..self.context_shared_reason_len]);
                self.context_shared_seq += 1;
            },
            .frame_inline => {
...
// ---- SPIKE(pac): temporary stage, run with SKETERM_SMOKE_WEB_PAC_ONLY=1 ----
const pac_view_a: u32 = 15;
const pac_view_b: u32 = 16;

/// Build `spike-pac-inline:<script>` (or the data-url form) routing
/// route-a.example -> `pa`, route-b.example -> `pb`, everything DIRECT.
/// A zero port means DIRECT for that host.
fn pacPref(buf: []u8, use_data_url: bool, pa: u16, pb: u16) []const u8 {
    var script_buf: [512]u8 = undefined;
    var w: usize = 0;
    var acc = std.fmt.bufPrint(&script_buf, "function FindProxyForURL(url, host) {{ ", .{}) catch unreachable;
    w += acc.len;
    if (pa != 0) {
        acc = std.fmt.bufPrint(script_buf[w..], "if (host == \"route-a.example\") return \"SOCKS5 127.0.0.1:{d}\"; ", .{pa}) catch unreachable;
        w += acc.len;
    }
    if (pb != 0) {
        acc = std.fmt.bufPrint(script_buf[w..], "if (host == \"route-b.example\") return \"SOCKS5 127.0.0.1:{d}\"; ", .{pb}) catch unreachable;
        w += acc.len;
    }
    acc = std.fmt.bufPrint(script_buf[w..], "return \"DIRECT\"; }}", .{}) catch unreachable;
    w += acc.len;
    const script = script_buf[0..w];
    if (!use_data_url) return std.fmt.bufPrint(buf, "spike-pac-inline:{s}", .{script}) catch unreachable;
    var b64_buf: [1024]u8 = undefined;
    const enc = std.base64.standard.Encoder;
    const b64 = enc.encode(b64_buf[0..enc.calcSize(script.len)], script);
    return std.fmt.bufPrint(buf, "spike-pac-url:data:application/x-ns-proxyautoconfig;base64,{s}", .{b64}) catch unreachable;
}

fn pacReset(a: *ProxyProbe, b: *ProxyProbe) void {
    a.route_a.store(false, .release);
    a.route_b.store(false, .release);
    b.route_a.store(false, .release);
    b.route_b.store(false, .release);
}

fn pacWaitBoth(cl: *Client, a: *std.atomic.Value(bool), b: *std.atomic.Value(bool), ms: i64) bool {
    const deadline = nowMs() + ms;
    while (nowMs() < deadline) {
        if (a.load(.acquire) and b.load(.acquire)) return true;
        cl.pump(50);
    }
    return false;
}

/// SPIKE: does a CEF 151 request context accept a PAC-script proxy pref,
/// route per-URL from it, replace it at runtime, and fail CLOSED?
fn runPacSpikeStage(gpa: std.mem.Allocator, exe: [*:0]const u8, dir: []const u8) void {
    var sock_buf: [96]u8 = undefined;
    const sock = std.fmt.bufPrintZ(&sock_buf, "{s}/pac.sock", .{dir}) catch fail("pac socket path");
    var cache_buf: [128]u8 = undefined;
    const cache = std.fmt.bufPrintZ(&cache_buf, "{s}/cache-pac", .{dir}) catch fail("pac cache path");
    const pid = spawnHelper(exe, sock.ptr, cache.ptr, "--ozone-platform=headless", null, false);
    g_pid = pid;
    var cl = Client{ .gpa = gpa, .fd = connectWithRetry(sock.ptr, sock.len) };
    cl.send(proto.Hello{ .proto = proto.PROTO_VERSION, .client_name = "smoke-web-pac" });
    {
        const deadline = nowMs() + 20_000;
        while (cl.ack_proto == 0 and nowMs() < deadline) cl.pump(100);
    }
    if (cl.ack_proto != proto.PROTO_VERSION) fail("pac spike: no hello_ack");

    var http = HttpProbe{};
    if (!http.start()) fail("pac spike: could not start the loopback HTTP probe");
    defer http.shutdown();
    var probe_a = ProxyProbe{ .tunnel_port = http.lis.port };
    var probe_b = ProxyProbe{ .tunnel_port = http.lis.port };
    if (!probe_a.start() or !probe_b.start()) fail("pac spike: could not start the SOCKS5 tunnels");
    defer probe_a.shutdown();
    defer probe_b.shutdown();

    const ctx_id = proto.EPHEMERAL_CTX_BASE + 40;
    var pref_buf: [2048]u8 = undefined;

    // (1) Which form does the preference manager accept? Try the data:
    // URL first; the helper prints ACCEPTED/REFUSED for each attempt and
    // rolls the context back when refused, so a failed ViewCreate is the
    // machine-readable half of the same answer.
    // SKETERM_SMOKE_WEB_PAC_INLINE=1 tries the inline form FIRST, so both
    // candidate forms can be measured on their own.
    const try_url_first = c.getenv("SKETERM_SMOKE_WEB_PAC_INLINE") == null;
    const url_form = pacPref(&pref_buf, try_url_first, probe_a.lis.port, probe_b.lis.port);
    cl.send(proto.ContextCreate{ .id = ctx_id, .ephemeral = 1, .name = "pacspike", .proxy = url_form });
    cl.send(proto.ViewCreate{ .view = pac_view_a, .w = 320, .h = 240, .scale_x1000 = 1000, .context = ctx_id });
    var accepted_form: []const u8 = if (try_url_first) "pac_url(data:)" else "pac_script(inline)";
    if (cl.waitSeq(&cl.view_create_fail_seq, 0, 4_000)) {
        std.debug.print("SPIKE(pac): the pac_url data: form was rejected; trying the inline form\n", .{});
        accepted_form = if (try_url_first) "pac_script(inline)" else "pac_url(data:)";
        const inline_form = pacPref(&pref_buf, !try_url_first, probe_a.lis.port, probe_b.lis.port);
        cl.send(proto.ContextCreate{ .id = ctx_id, .ephemeral = 1, .name = "pacspike", .proxy = inline_form });
        cl.send(proto.ViewCreate{ .view = pac_view_a, .w = 320, .h = 240, .scale_x1000 = 1000, .context = ctx_id });
        if (cl.waitSeq(&cl.view_create_fail_seq, 1, 4_000))
            fail("pac spike: NEITHER pac form was accepted by the preference manager");
    }
    std.debug.print("SPIKE(pac): accepted form = {s}\n", .{accepted_form});

    // (2) Per-URL routing, concurrently, in ONE context.
    var a_buf: [96]u8 = undefined;
    var b_buf: [96]u8 = undefined;
    const url_a = std.fmt.bufPrint(&a_buf, "http://route-a.example:{d}/?one", .{http.lis.port}) catch unreachable;
    const url_b = std.fmt.bufPrint(&b_buf, "http://route-b.example:{d}/?one", .{http.lis.port}) catch unreachable;
    cl.send(proto.ViewCreate{ .view = pac_view_b, .w = 320, .h = 240, .scale_x1000 = 1000, .context = ctx_id });
    cl.send(proto.Navigate{ .view = pac_view_a, .url = url_a });
    cl.send(proto.Navigate{ .view = pac_view_b, .url = url_b });
    if (!pacWaitBoth(&cl, &probe_a.route_a, &probe_b.route_b, 30_000)) {
        std.debug.print("SPIKE(pac): route wait timeout: A(a={},b={}) B(a={},b={}) http={d}\n", .{
            probe_a.route_a.load(.acquire), probe_a.route_b.load(.acquire),
            probe_b.route_a.load(.acquire), probe_b.route_b.load(.acquire),
            http.served.load(.acquire),
        });
        fail("pac spike: the two PAC routes did not both fire");
    }
    if (probe_a.route_b.load(.acquire) or probe_b.route_a.load(.acquire))
        fail("pac spike: a probe saw the OTHER host (the PAC mapping leaked)");
    std.debug.print("SPIKE(pac): concurrent per-URL routing OK: A saw route-a only, B saw route-b only\n", .{});

    // (3) REPLACE the script at runtime: route-a now goes through B.
    pacReset(&probe_a, &probe_b);
    var swapped2_buf: [2048]u8 = undefined;
    const use_url = std.mem.eql(u8, accepted_form, "pac_url(data:)");
    const swapped_pref = pacPref(&swapped2_buf, use_url, probe_b.lis.port, probe_b.lis.port);
    cl.send(proto.ContextCreate{ .id = ctx_id, .ephemeral = 1, .name = "pacspike", .proxy = swapped_pref });
    var a2_buf: [96]u8 = undefined;
    const url_a2 = std.fmt.bufPrint(&a2_buf, "http://route-a.example:{d}/?two", .{http.lis.port}) catch unreachable;
    cl.send(proto.Navigate{ .view = pac_view_a, .url = url_a2 });
    {
        const deadline = nowMs() + 30_000;
        while (nowMs() < deadline and !probe_b.route_a.load(.acquire)) cl.pump(50);
    }
    if (!probe_b.route_a.load(.acquire))
        fail("pac spike: the replacement script never took effect (probe B never saw route-a)");
    if (probe_a.route_a.load(.acquire))
        fail("pac spike: probe A still received route-a after the replacement");
    std.debug.print("SPIKE(pac): runtime replacement OK: route-a moved from probe A to probe B\n", .{});

    // (4) pac_mandatory fail-closed: point route-a at a DEAD proxy port
    // and confirm the load never reaches the origin directly.
    // Both hosts map to a DEAD proxy port, so ANY request that reaches
    // the origin necessarily bypassed the proxy: no probe can relay it.
    const dead_pref = pacPref(&pref_buf, use_url, 1, 1);
    cl.send(proto.ContextCreate{ .id = ctx_id, .ephemeral = 1, .name = "pacspike", .proxy = dead_pref });
    // Let the preference change settle before the navigation races it.
    {
        const settle = nowMs() + 3000;
        while (nowMs() < settle) cl.pump(50);
    }
    pacReset(&probe_a, &probe_b);
    const served_before = http.served.load(.acquire);
    cl.resetTitle();
    var a3_buf: [96]u8 = undefined;
    const url_a3 = std.fmt.bufPrint(&a3_buf, "http://route-a.example:{d}/?three", .{http.lis.port}) catch unreachable;
    cl.send(proto.Navigate{ .view = pac_view_a, .url = url_a3 });
    {
        const deadline = nowMs() + 12_000;
        while (nowMs() < deadline) cl.pump(50);
    }
    const served_after = http.served.load(.acquire);
    const leaked_direct = served_after != served_before;
    std.debug.print("SPIKE(pac): dead-proxy result: http served {d}->{d}, title=\"{s}\", A(a={}) B(a={})\n", .{
        served_before, served_after, cl.titleSlice(),
        probe_a.route_a.load(.acquire), probe_b.route_a.load(.acquire),
    });
    if (leaked_direct) fail("pac spike: a dead mandatory proxy FELL BACK TO DIRECT (not fail-closed)");
    std.debug.print("SPIKE(pac): pac_mandatory held: the dead-proxy load did NOT go direct\n", .{});

    cl.send(proto.ViewDestroy{ .view = pac_view_a });
    cl.send(proto.ViewDestroy{ .view = pac_view_b });
    cl.teardown_allow_close = true;
    {
    ...
    cl.have_view = false;
    cl.deinit();
    reapHelperTolerant(pid, "pac spike teardown", 30_000);
    pass("pac spike (see the SPIKE(pac) lines above)");
}
// ---- end SPIKE(pac) --------------------------------------------------

/// Probe CEF's shared-storage request contexts against two live SOCKS routes.
...
fn runSharedContextStage(gpa: std.mem.Allocator, exe: [*:0]const u8, dir: []const u8) void {
    var sock_buf: [96]u8 = undefined;
    const sock = std.fmt.bufPrintZ(&sock_buf, "{s}/xs.sock", .{dir}) catch fail("shared-context socket path");
    var cache_buf: [128]u8 = undefined;
    const cache = std.fmt.bufPrintZ(&cache_buf, "{s}/cache-shared-context", .{dir}) catch fail("shared-context cache path");
    _ = c.setenv("SKETERM_WEB_PROBE_SHARED_PROXY", "1", 1);
    const pid = spawnHelper(exe, sock.ptr, cache.ptr, "--ozone-platform=headless", null, false);
    _ = c.unsetenv("SKETERM_WEB_PROBE_SHARED_PROXY");
    g_pid = pid;
    var cl = Client{ .gpa = gpa, .fd = connectWithRetry(sock.ptr, sock.len) };
    cl.send(proto.Hello{ .proto = proto.PROTO_VERSION, .client_name = "smoke-web-shared-context" });
    {
        const deadline = nowMs() + 20_000;
        while (cl.ack_proto == 0 and nowMs() < deadline) cl.pump(100);
    }
    if (cl.ack_proto != proto.PROTO_VERSION) fail("stage 27b shared routing: no hello_ack");
    if (!cl.ack_contexts or !cl.ack_contexts_fail_closed or !cl.ack_contexts_shared_storage)
        fail("stage 27b shared routing: helper lacks strict shared-context support");

    var http = HttpProbe{ .body = jar_page };
    if (!http.start()) fail("stage 27b shared routing: could not start the loopback HTTP probe");
    defer http.shutdown();
    var probe_a = ProxyProbe{ .tunnel_port = http.lis.port };
    var probe_b = ProxyProbe{ .tunnel_port = http.lis.port };
    if (!probe_a.start() or !probe_b.start()) fail("stage 27b shared routing: could not start the SOCKS5 tunnels");
    defer probe_a.shutdown();
    defer probe_b.shutdown();

    var proxy_a_buf: [64]u8 = undefined;
    var proxy_b_buf: [64]u8 = undefined;
    const proxy_a = std.fmt.bufPrint(&proxy_a_buf, "socks5://127.0.0.1:{d}", .{probe_a.lis.port}) catch unreachable;
    const proxy_b = std.fmt.bufPrint(&proxy_b_buf, "socks5://127.0.0.1:{d}", .{probe_b.lis.port}) catch unreachable;
    const route_ctx_a = proto.EPHEMERAL_CTX_BASE + 30;
    const route_ctx_b = proto.EPHEMERAL_CTX_BASE + 31;

    cl.send(proto.ContextCreate{
        .id = route_ctx_a,
        .ephemeral = 1,
        .name = "shared-route-root",
        .proxy = proxy_a,
    });
    const seq = cl.context_shared_seq;
    cl.send(proto.ContextCreateShared{
        .req = 2702,
        .id = route_ctx_b,
        .share_with = route_ctx_a,
        .proxy = proxy_b,
    });
    if (!cl.waitSeq(&cl.context_shared_seq, seq, 10_000))
        fail("stage 27b shared routing: context B creation was not answered");
    if (cl.context_shared_req != 2702 or cl.context_shared_ok == 0) {
        std.debug.print("smoke-web: shared context B said \"{s}\"\n", .{cl.context_shared_reason[0..cl.context_shared_reason_len]});
        fail("stage 27b shared routing: context B creation failed or was miscorrelated");
    }

    var set_buf: [96]u8 = undefined;
    var plain_buf: [96]u8 = undefined;
    const set_url = std.fmt.bufPrint(&set_buf, "http://shared-route.example:{d}/?set&route-a", .{http.lis.port}) catch unreachable;
    const plain_url = std.fmt.bufPrint(&plain_buf, "http://shared-route.example:{d}/?plain&route-b", .{http.lis.port}) catch unreachable;

    cl.send(proto.ViewCreate{ .view = shared_route_view_a, .w = 320, .h = 240, .scale_x1000 = 1000, .context = route_ctx_a });
    cl.send(proto.Navigate{ .view = shared_route_view_a, .url = set_url });
    if (!cl.waitTitle("jarA:", 20_000)) {
        std.debug.print(
            "smoke-web: shared route timeout: title=\"{s}\" http={d} proxyA(got={},a={},b={}) proxyB(got={},a={},b={})\n",
            .{
                cl.titleSlice(),
                http.served.load(.acquire),
                probe_a.got.load(.acquire),
                probe_a.route_a.load(.acquire),
                probe_a.route_b.load(.acquire),
                probe_b.got.load(.acquire),
                probe_b.route_a.load(.acquire),
                probe_b.route_b.load(.acquire),
            },
        );
        fail("stage 27b shared routing: context A navigation never completed");
    }
    if (std.mem.indexOf(u8, cl.titleSlice(), jar_cookie) == null)
        fail("stage 27b shared routing: context A could not store its cookie");

    cl.send(proto.ViewCreate{ .view = shared_route_view_b, .w = 320, .h = 240, .scale_x1000 = 1000, .context = route_ctx_b });
    cl.send(proto.Navigate{ .view = shared_route_view_b, .url = plain_url });
    if (!cl.waitTitle("jarB:", 20_000))
        fail("stage 27b shared routing: context B never loaded through proxy B");
    if (std.mem.indexOf(u8, cl.titleSlice(), jar_cookie) == null)
        fail("stage 27b shared routing: context B did not receive context A's cookie");

    // CEF 151 shares the preference manager too: installing proxy B moves BOTH
    // navigations to B. This is the exact negative invariant the phase needed
    // measured. If a future engine changes, this stage fails and the production
    // `IsSame` refusal above can be revisited against new evidence.
    if (probe_a.route_a.load(.acquire) or probe_a.route_b.load(.acquire) or
        !probe_b.route_a.load(.acquire) or !probe_b.route_b.load(.acquire))
    {
        std.debug.print(
            "smoke-web: shared route observation: proxyA(a={},b={}) proxyB(a={},b={})\n",
            .{
                probe_a.route_a.load(.acquire),
                probe_a.route_b.load(.acquire),
                probe_b.route_a.load(.acquire),
        ...
        fail("stage 27b shared routing: CEF's shared-preference result changed");
    }
    if (probe_b.port != http.lis.port)
        fail("stage 27b shared routing: proxy B targeted the wrong origin port");

    cl.send(proto.ViewDestroy{ .view = shared_route_view_a });
    cl.send(proto.ViewDestroy{ .view = shared_route_view_b });
    cl.teardown_allow_close = true;
    {
        const deadline = nowMs() + 3000;
        while (nowMs() < deadline) cl.pump(50);
    ...
    if (cl.fd >= 0) {
        cl.send(proto.ContextDestroy{ .id = route_ctx_b });
        cl.send(proto.ContextDestroy{ .id = route_ctx_a });
        const deadline = nowMs() + 1500;
        while (nowMs() < deadline and cl.fd >= 0) cl.pump(50);
    }
    cl.deinit();
    reapHelperTolerant(pid, "stage 27b shared routing", 30_000);
    pass("stage 27b invariant disproven (cookie shared; last proxy preference routes both contexts)");
}
...
pub fn main(init: std.process.Init.Minimal) u8 {
    ...
    if (c.getenv("SKETERM_SMOKE_WEB_PAC_ONLY") != null) {
        runPacSpikeStage(gpa, exe, dir);
        cleanup();
        _ = gpa_state.deinit();
        say("smoke-web: PASS (pac spike only)");
        return 0;
    }
    ...
    if (c.getenv("SKETERM_SMOKE_WEB_SHARED_CONTEXT_ONLY") != null) {
        runSharedContextStage(gpa, exe, dir);
        cleanup();
        if (gpa_state.deinit() == .leak) {
            say("smoke-web: FAIL leaked memory (see GPA report above)");
            return 1;
        }
        say("smoke-web: PASS (shared context only)");
        return 0;
    }
    ...
    // fails exactly like any other missing context and never gets a buffer.
    {
        ...
        if (!fc.ack_contexts or !fc.ack_contexts_fail_closed or !fc.ack_contexts_shared_storage)
            fail("stage 26 proxy refusal: helper lacks strict context support");
        const source_ctx = proto.EPHEMERAL_CTX_BASE + 12;
        const failed_ctx = proto.EPHEMERAL_CTX_BASE + 13;
        fc.send(proto.ContextCreate{
            .id = source_ctx,
            .ephemeral = 1,
            .name = "proxy-refusal-source",
            .proxy = "",
        });
        const shared_seq = fc.context_shared_seq;
        fc.send(proto.ContextCreateShared{
            .req = 2601,
            .id = failed_ctx,
            .share_with = source_ctx,
            .proxy = "socks5://127.0.0.1:9",
        });
        if (!fc.waitSeq(&fc.context_shared_seq, shared_seq, 10_000))
            fail("stage 26 proxy refusal: shared context creation was not answered");
        if (fc.context_shared_req != 2601 or fc.context_shared_ok != 0 or
            !std.mem.eql(u8, fc.context_shared_reason[0..fc.context_shared_reason_len], "proxy preference was refused"))
            fail("stage 26 proxy refusal: shared context failure was not correlated and described");
        ...
        if (fc.view_create_fail_view != egress_proxy_fail_view or fc.view_create_fail_context != failed_ctx)
        ...
        pass("stage 26 proxy refusal (correlated context failure, rollback, view creation fails closed)");
        fc.deinit();
