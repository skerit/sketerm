...
/// Correlated shared-storage routing-context creation with fail-closed proxy setup.
pub const CAP_CONTEXTS_SHARED_STORAGE = "contexts-shared-storage";
...
pub const Tag = enum(u8) {
    ...
    context_create_shared = 0x93,
    ev_context_create_shared = 0x94,
    sem_eval = 0xA0,
...
/// Create a client-owned fixed-proxy context sharing another context's storage.
// `req` is always echoed by `ev_context_create_shared`. `id` must be in the
// ephemeral partition, and `share_with` must name a non-global context.
pub const ContextCreateShared = struct {
    pub const tag: Tag = .context_create_shared;
    req: u32,
    id: u32,
    share_with: u32,
    proxy: []const u8,
...
/// Report the correlated result of `context_create_shared`.
// A failed context never enters the registry, so a view naming it fails closed.
pub const EvContextCreateShared = struct {
    pub const tag: Tag = .ev_context_create_shared;
    req: u32,
    ok: u8,
    reason: []const u8,
};
...
test "round-trip: container/context frames" {
    ...
    try roundTrip(ContextCreateShared, .{
        .req = 91,
        .id = EPHEMERAL_CTX_BASE + 4,
        .share_with = 3,
        .proxy = "socks5://127.0.0.1:19181",
    });
    try roundTrip(EvContextCreateShared, .{ .req = 91, .ok = 1, .reason = "" });
    try roundTrip(EvContextCreateShared, .{ .req = 92, .ok = 0, .reason = "proxy preference was refused" });
}
