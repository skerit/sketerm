...
// Host-side chrome ON that window (the Ctrl+right-click menu,
// screenshots, recording) -- likewise shared with the winstream
// renderer, so both backends offer the same rows in the same order.
...
pub const AppHost = struct {
    ...
    const Win = struct {
        ...
        /// mechanism (`rmenu.popupHostMenu`), which winapp.zig uses too; the
        ...
        fn showHostMenu(self: *Win, x: f64, y: f64) void {
            ...
            for (plan, 0..) |item, i| {
                rows[i] = .{
                    .label = rmenu.hostMenuLabel(item),
            ...
            rmenu.popupHostMenu(self.picture, x, y, rows[0..plan.len]);
        }
